...
namespace Mang.ServiceBase.Impl
{
    ...
    public abstract class ReadSerivce<TEntity, TDto, TListDto, TGetListInput> : BaseService,
        ...
        where TGetListInput : GetPageDto
    {
        ...
        public virtual async Task<PageResultDto<TListDto>> GetAsync(TGetListInput input)
        {
            ...
            var pageEntity = await query.Count(out var total)
                .Page(input.Index, input.Size)
                .ToListAsync();
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        protected virtual ISelect<TEntity> DefaultSorting(ISelect<TEntity> query)
        {
